...
- 职责：提供 `graceful-session`，作为 mutter 启动后的用户 session 根进程；负责读取 session 定义、设置基础环境变量、启动默认桌面组件、拉起 XDG Autostart 启动项，并随核心组件退出结束 session。
...
- 用户可见行为：Display Manager 中可出现 `Graceful` Wayland session；session 入口先启动 `mutter --wayland --display-server -- /usr/bin/graceful-session`，`graceful-session` 默认启动 `ibus-daemon --daemonize --xim`、`ibus engine rime`、安装目录下的 `graceful-desktop` 和 `graceful-panel`；核心组件启动成功后扫描 XDG Autostart 并拉起启动项；`graceful-session -- [COMMAND...]` 会保留单命令调试模式并等待 `COMMAND` 退出。
...
- 兼容性要求：构建依赖 `glib-2.0`、`gobject-2.0`、`gio-2.0`；安装 `graceful.desktop` 到 `${datadir}/wayland-sessions`。
