...
- 职责：提供 `graceful-session`，作为登录成功后的用户 session 根进程；负责读取 session 定义、设置基础环境变量、启动核心命令并以核心命令退出作为 session 结束。
...
- 用户可见行为：Display Manager 中可出现 `Graceful` session；`graceful-session -- [COMMAND...]` 会将 `COMMAND` 作为 session 核心进程运行并等待其退出。
...
- 默认规则：未提供命令时优先使用 `GRACEFUL_SESSION_COMMAND`，否则默认运行 `graceful-desktop`；设置 `DESKTOP_SESSION`、`GDMSESSION`、`XDG_CURRENT_DESKTOP`。
...
| 单元测试 | `ctest --test-dir build --output-on-failure` | 覆盖 session 定义、环境变量、子进程退出状态 |
