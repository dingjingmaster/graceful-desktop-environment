...
- 非职责：不提供多 panel，不提供动态插件系统，不读取 GSettings/配置文件，不实现 applet 持久化布局，不实现真实任务列表、系统托盘、网络/音量/电源控制。
- 相关代码路径：`panel/`。
- 相关长期文档：`docs/overview-product.md`、`docs/overview-product-dev.md`。

## 2. 当前行为

- 用户可见行为：`graceful-panel` 启动 GTK4 应用，创建无边框 panel 窗口；左侧显示 `Graceful` 菜单按钮和终端/文件/设置启动器占位；中间显示任务区占位；右侧显示网络、音量、电源图标和本地时间。
...
- 配置/接口/数据：无运行时配置；所有 item 为内置 GObject/GTK widget。
- 默认规则：单 panel、固定高度 40px、底部 dock top-level 窗口；时钟以 `HH:MM` 格式显示本地时间。
...
- 性能/稳定性要求：第一阶段只有时钟定时刷新；GObject 引用和定时器需要正确释放。
...
| 单元测试 | `ctest --test-dir build --output-on-failure` | 覆盖时钟模型格式化 |
