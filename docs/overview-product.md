...
- 核心功能：当前已实现 LightDM greeter 的最小图形登录入口，以及登录后的 `graceful-session` 根进程最小闭环。
...
8. 核心命令退出后，`graceful-session` 退出，用户会话结束。
```
...
- Session 规则：第一版 `graceful-session` 不提供完整 GNOME SessionManager D-Bus API；默认核心命令为 `graceful-desktop`，该进程后续实现前真实登录可能立即结束。
